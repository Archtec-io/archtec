...
function archtec_teleport.tpr_create(name, target)
	minetest.chat_send_player(name, C("#FF8800", S("[tpr] Teleport request sent. It will timeout in @1 seconds.", timeout)))
	minetest.chat_send_player(target, C("#FF8800", S("[tpr] @1 is requesting to teleport to you. Run '/ok' to accept.", name)))
...
function archtec_teleport.tp2me_create(name, target)
	minetest.chat_send_player(name, C("#FF8800", S("[tp2me] Teleport request sent. It will timeout in @1 seconds.", timeout)))
	minetest.chat_send_player(target, C("#FF8800", S("[tp2me] @1 is requesting to teleport you to them. Run '/ok' to accept.", name)))
	...
	minetest.after(timeout, function()
		if archtec_teleport.tp2me[target] and archtec_teleport.tp2me[target].name == name and archtec_teleport.tp2me[target].created == time then
			minetest.chat_send_player(name, C("#FF8800", S("[tp2me] Your request to teleport @1 to you timed-out.", target)))
			minetest.chat_send_player(target, C("#FF8800", S("[tp2me] Teleport request to teleport you to @1 timed-out.", name)))
...
function archtec_teleport.tpr_accept(name, target) -- name gets teleported; target /ok
	...
	if not pos then
		minetest.chat_send_player(name, C("#FF0000", S("[tpr] @1 tried to accept the teleport request but isn't at a safe spot!", target)))
		minetest.chat_send_player(target, C("#FF0000", S("[tpr] You can't accept the teleport request because you aren't at a safe spot!")))
		return
	end
...
function archtec_teleport.tp2me_accept(name, target) -- name nothing; target /ok and gets teleported
	...
	if not pos then
		minetest.chat_send_player(name, C("#FF0000", S("[tp2me] @1 tried to accept the teleport request but you aren't at a safe spot!", target)))
		minetest.chat_send_player(target, C("#FF0000", S("[tp2me] You can't accept the teleport request because @1 isn't at a safe spot!", name)))
		return
	end
	...
	minetest.chat_send_player(name, C("#FF8800", S("[tp2me] @1 accepted your request to teleport them to you.", target)))
...
minetest.register_chatcommand("ok", {
	...
	func = function(name, param)
		...
		if not archtec_teleport.tpr[name] and not archtec_teleport.tp2me[name] then
			minetest.chat_send_player(name, C("#FF0000", S("[tpr] There is currently no teleport request which you could accept!")))
			return
		end
