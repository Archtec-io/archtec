if not minetest.get_modpath("techage") then
	return
end
local S = archtec.S
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if not minetest.check_player_privs(name, "forceload") then
			minetest.chat_send_player(
				name,
				minetest.colorize("#FF0000", S("[Forceload Restriction]: 'forceload' priv required to use this node!"))
			)
			return
		else
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if not minetest.check_player_privs(name, "forceload") then
			...
			minetest.chat_send_player(
				...
				minetest.colorize(
					"#00BD00",
					S("Congratulations! You have been granted the '@1' privilege.", "forceload")
				)
			)
