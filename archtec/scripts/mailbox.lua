...
local function img_col(stack)
	local def = core.registered_items[stack]
	if not def then
		return ""
	end
...
local function can_modify_mailbox(owner, name)
	return owner == name or core.get_player_privs(name).staff
end

function mailbox.get_formspec(pos, owner, fs_type)
	local is_letterbox = "false"
	if core.get_node(pos).name == "mailbox:letterbox" then
		is_letterbox = "true"
	end
...
function mailbox.unrent(pos, player)
	...
	if can_modify_mailbox(meta:get_string("owner"), name) then
		...
		if pos_sign and core.get_node(pos_sign).name == "basic_signs:sign_wall_steel_white_black" then
...
core.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.unrent then
		...
		if inv:is_empty("mailbox") then
			...
			core.chat_send_player(player:get_player_name(), C("#FF0000", S("Your mailbox is not empty!")))
		end
	end
...
function mailbox.after_place_node(pos, placer)
	...
	archtec_playerdata.set(name, "mailbox", core.pos_to_string(pos))
	...
	local node = core.get_node(pos)
	local pos_sign = get_sign_pos(pos, node.param2)
	local prev_node_name = core.get_node(pos_sign).name

	if pos_sign and (prev_node_name == "air" or prev_node_name == "basic_signs:sign_wall_steel_white_black") then
		local meta_sign = core.get_meta(pos_sign)

		core.set_node(pos_sign, {name = "basic_signs:sign_wall_steel_white_black", param2 = node.param2})
...
function mailbox.on_rightclick_free(pos, _, clicker)
	...
	if archtec_playerdata.get(name, "mailbox") ~= "" then
		core.chat_send_player(name, C("#FF0000", S("You have already rented a mailbox at @1!", archtec_playerdata.get(name, "mailbox"))))
		return
	end
...
function mailbox.on_rightclick(pos, _, clicker)
	...
	if name == owner or (core.get_player_privs(name).staff and clicker:get_player_control().aux1) then
		local spos = pos.x .. "," .. pos.y .. "," .. pos.z
		core.show_formspec(name, "mailbox:mailbox_" .. spos, mailbox.get_formspec(pos, owner, 1))
	else
		core.show_formspec(name, "mailbox:mailbox", mailbox.get_formspec(pos, owner, 0))
	end
end
...
function mailbox.allow_metadata_inventory_put(pos, listname, index, stack, player)
	...
	if listname == "drop" then
		...
		if core.get_node(pos).name == "mailbox:letterbox" and
		...
		if inv:room_for_item("mailbox", stack) then
			...
			core.chat_send_player(name, C("#FF0000", S("Mailbox full!")))
			return 0
		end
	elseif listname == "mailbox" and core.get_player_privs(name).staff then -- staff can put things back
...
core.register_node(":mailbox:mailbox", {
...
core.register_node(":mailbox:mailbox_free", {
...
core.register_node(":mailbox:letterbox", {
	tiles = {
