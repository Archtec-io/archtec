...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if fields.news and fields.news:sub(1, 12) == "action:link_" then
		...
		if link == "website" then
			url = archtec.links.website
		end
		if link == "discord" then
			url = archtec.links.discord
		end
		if link == "matrix" then
			url = archtec.links.matrix
		end
		...
		if url ~= nil then
			...
			minetest.chat_send_player(
				name,
				minetest.colorize("#FF8800", S("Ctrl + Click the link to open your browser:")) .. " " .. url
			)
		end
	end
