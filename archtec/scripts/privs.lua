...
function archtec.check_areas_high_limit(name, privs)
	...
	if playtime > archtec.big_areas_playtime then
		...
		minetest.chat_send_player(
			name,
			C("#00BD00", S("Congratulations! You have been granted the '@1' privilege.", "areas_high_limit"))
		)
		...
		minetest.chat_send_player(
			name,
			C("#FF0000", S("You do not have @1 hours (or more) playtime", archtec.big_areas_playtime))
		)
		return
	end
end
