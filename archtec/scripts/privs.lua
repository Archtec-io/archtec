...
minetest.register_chatcommand("request_lava", {
	...
	func = function(player, param)
        if minetest.check_player_privs(player, "adv_buckets") then
            minetest.chat_send_player(player, C("#00BD00", "[request_lava] You already have the 'adv_buckets' privilege"))
            return
        end
        ...
        if playtime > 180000 then -- 50 h playtime
            ...
            minetest.chat_send_player(player, C("#00BD00", "[request_lava] Congratulations! You have been granted the 'adv_buckets' privilege"))
            notifyTeam("[request_lava] Granted '" .. player .. "' the 'adv_buckets' priv")
        else
            minetest.chat_send_player(player, C("#FF0000", "[request_lava] You do not have 50 hours (or more) playtime."))
        end
	end
})
...
minetest.register_chatcommand("request_areas_high_limit", {
	...
	func = function(player, param)
        if minetest.check_player_privs(player, "areas_high_limit") then
            minetest.chat_send_player(player, C("#00BD00", "[request_areas_high_limit] You already have the 'areas_high_limit' privilege"))
            return
        end
        local playtime = archtec_playerdata.get(player, "playtime") or 0
        if playtime > 108000 then -- 30 h playtime
            grant_priv(player, "areas_high_limit")
            minetest.chat_send_player(player, C("#00BD00", "[request_areas_high_limit] Congratulations! You have been granted the 'areas_high_limit' privilege"))
            notifyTeam("[request_areas_high_limit] Granted '" .. player .. "' the 'areas_high_limit' priv")
        else
            minetest.chat_send_player(player, C("#FF0000", "[request_areas_high_limit] You do not have 30 hours (or more) playtime."))
        end
	end
})